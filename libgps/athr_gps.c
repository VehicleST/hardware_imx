...
#define GNSS_TTY "/dev/ttymxc1"
#define GNSS_SPEED 115200
...
nmea_reader_parse( NmeaReader*  r )
{
    ...
    if ( !memcmp(tok.p, "GGA", 3) ) {
        ...
        Token  tok_isPix         = nmea_tokenizer_get(tzer,6);//Quality indicator 0 -> No Fix, 1 -> Fix Available 2->Differential Mode
        Token  tok_altitude      = nmea_tokenizer_get(tzer,9);
        Token  tok_altitudeUnits = nmea_tokenizer_get(tzer,10);
        
        if (tok_isPix.p[0] > '0') { // value is '0' mostly
        ...
        Token  tok_fixstaus      = nmea_tokenizer_get(tzer,6); // 'A' valid data, 'V' invalid data

        if (tok_fixstaus.p[0] == 'A') { // 'V' from what i noticed
        ...
        Token  tok_fixStatus   = nmea_tokenizer_get(tzer, 2); //Mode '1' -> Fix not available, '2' -> 2D position fix or Altitude hold mode, '3'-> 3D position fix. 
        int i;
        
        memset(r->sv_used_in_fix, 0, sizeof(MAX_SV_PRN));   
        
        if (tok_fixStatus.p[0] != '\0' && tok_fixStatus.p[0] != '1') { // '1' from what i have noticed
            
            Token  tok_accuracy      = nmea_tokenizer_get(tzer, 15);// pdop
            
            nmea_reader_update_accuracy(r, tok_accuracy);   // mostly NULL
        ...
        if (tok_fixStatus.p[0] != '\0' && tok_fixStatus.p[0] != 'N')// 'N' not a valid data from what i noticed
        {
        ...
        //ZDA message contains UTC time, day, month, year and the local time zone
        // Mostly all '/0' in our case 
